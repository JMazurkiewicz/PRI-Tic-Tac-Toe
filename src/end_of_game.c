...
static bool isThisTie(const Game* game);
static Player whoWon(const Game* game);

static Player checkColumns(const Game* game);
static Player checkSingleColumn(const Game* game, unsigned columnIndex);

static Player checkRows(const Game* game);
static Player checkSingleRow(const Game* game, unsigned rowIndex);

static Player checkSlants(const Game* game);
static Player checkHorizontalSlants(const Game* game);
static Player checkVerticalSlants(const Game* game);

static Player checkSingleSlant(const Game* game, Vector2 start, bool isReversed);
